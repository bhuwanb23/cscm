...
async function demoFeatureStorage() {
  ...
  try {
    ...
    const inventoryLevels = {
      'STORE-NYC-001': {
        sku_apple_iphone_15: 25,
        sku_samsung_galaxy_s24: 18,
        sku_google_pixel_8: 30,
      },
      'STORE-LA-001': {
        sku_apple_iphone_15: 32,
        sku_samsung_galaxy_s24: 22,
        sku_google_pixel_8: 28,
      },
      'STORE-CHI-001': {
        sku_apple_iphone_15: 19,
        sku_samsung_galaxy_s24: 15,
        sku_google_pixel_8: 35,
      },
    };
    ...
    console.log(
      `📈 Normalized sales data: [${normalizedSales.map((x) => x.toFixed(2)).join(', ')}]`
    );

    // Apply standardization to supplier lead times
    const standardizedLeadTimes = featureStorage.applyTransformation(
      'supplier_lead_times',
      'standardize'
    );
    console.log(
      `📊 Standardized lead times: [${standardizedLeadTimes.map((x) => x.toFixed(2)).join(', ')}]`
    );

    // Apply scaling to sales data
    const scaledSales = featureStorage.applyTransformation('weekly_sales_trend', 'scale', {
      factor: 0.01,
    });
    ...
    console.log(
      '💡 This demonstrates a complete feature storage workflow for supply chain analytics.'
    );
  } catch (error) {
