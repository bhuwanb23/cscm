...
class StoreAgent {
  ...
  async handleInventoryUpdate(topic, message) {
    try {
      ...
      console.error(
        `Store Agent ${this.storeId}: Failed to handle inventory update:`,
        error.message
      );
    }
  }
  ...
  async handleSalesData(topic, message) {
    try {
      ...
      if (message.productId && message.sales !== undefined) {
        ...
        this.state.historicalSales[message.productId] = this.state.historicalSales[
          message.productId
  ...
  async handleProductAttributes(topic, message) {
    try {
      ...
      console.error(
        `Store Agent ${this.storeId}: Failed to handle product attributes:`,
        error.message
      );
    }
  }
  ...
  async handleSupplierInfo(topic, message) {
    try {
      ...
      console.error(
        `Store Agent ${this.storeId}: Failed to handle supplier information:`,
        error.message
      );
    }
  }
      ...
      if (message.productId && message.orderId) {
        ...
        if (message.receivedQuantity !== undefined) {
          ...
          this.updateInventory(
            message.productId,
            currentInventory.quantity + message.receivedQuantity
          );
        }
      }
    } catch (error) {
      console.error(
        `Store Agent ${this.storeId}: Failed to handle restock response:`,
        error.message
      );
    }
  }
  ...
  async handleDemandForecast(topic, message) {
    try {
      ...
      console.error(
        `Store Agent ${this.storeId}: Failed to handle demand forecast:`,
        error.message
      );
    }
  }
  ...
  async updateDemandForecast(productId) {
    try {
      ...
      console.log(
        `Store Agent ${this.storeId}: Updated demand forecast for product ${productId}: ${forecast.expectedDemand} units`
      );
      this.makeRestockingDecision(productId);
    } catch (error) {
      console.error(
        `Store Agent ${this.storeId}: Failed to update demand forecast:`,
        error.message
      );
    }
  }
  ...
  async makeRestockingDecision(productId) {
    try {
      ...
      const optimizationResult = await this.inventoryOptimizer.optimize(
        productId,
        currentStock,
        forecast,
        productAttrs,
      ...
      if (this.inventoryOptimizer.needsRestock(currentStock, reorder_point)) {
        ...
        console.log(
          `Store Agent ${this.storeId}: Low stock for product ${productId}. Current: ${currentStock}, Reorder Point: ${reorder_point}. Requesting ${optimal_quantity} units`
        );
      ...
      console.error(
        `Store Agent ${this.storeId}: Failed to make restocking decision:`,
        error.message
      );
    }
  }

  async generateRestockingRecommendation(
    productId,
    optimalQuantity,
    reorderPoint,
    forecast,
    ...
    try {
      ...
      console.error(
        `Store Agent ${this.storeId}: Failed to generate restocking recommendation:`,
        error.message
      );
      return {
   ...
   */
  calculateOptimalOrderQuantityWithNewsvendor(
    productId,
    forecast,
    currentStock,
    holdingCost,
    ...
    try {
      ...
      console.error(
        `Store Agent ${this.storeId}: Failed to calculate optimal order quantity with Newsvendor model:`,
        error.message
      );
  ...
  updateInventory(productId, quantity) {
    try {
      ...
      console.log(
        `Store Agent ${this.storeId}: Updated inventory for product ${productId} to ${quantity} units`
      );
    } catch (error) {
