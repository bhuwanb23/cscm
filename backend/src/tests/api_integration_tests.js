  ...
  routingOptimization: {
    ...
    deliveries: [
      ...
      },
      {
        ...
        demand: 150,
        time_window: { start: '10:00', end: '13:00' },
      },
      {
        id: 'D3',
        location: { lat: 12.9719, lng: 77.5949 },
        demand: 200,
        time_window: { start: '11:00', end: '14:00' },
      },
    ],
  },
...
async function testDemandForecastingAPI() {
  ...
  try {
    const response = await axios.post(
      ...
      testScenarios.demandForecasting
    );
...
async function testInventoryOptimizationAPI() {
  ...
  try {
    const response = await axios.post(
      ...
      testScenarios.inventoryOptimization
    );
    ...
    logTestResult(
      'Inventory Optimization API Call',
      isValid,
...
async function testRoutingOptimizationAPI() {
  ...
  try {
    const response = await axios.post(
      `${BASE_URL}/api/v1/routing/optimize`,
      testScenarios.routingOptimization
    );

    const isValid =
      response.status === 200 && response.data.routes && Array.isArray(response.data.routes);

    logTestResult(
      'Routing Optimization API Call',
      isValid,
...
async function testSupplierRiskAPI() {
  ...
  try {
    const response = await axios.post(
      `${BASE_URL}/api/v1/supplier/risk`,
      testScenarios.supplierRisk
    );
    ...
    logTestResult(
      'Supplier Risk Assessment API Call',
      isValid,
...
async function testAnomalyDetectionAPI() {
  ...
  try {
    const response = await axios.post(
      ...
      testScenarios.anomalyDetection
    );

    const isValid =
      response.status === 200 && response.data.anomalies && Array.isArray(response.data.anomalies);
