...
describe('Feature Storage Tests', () => {
  ...
  test('should update existing features', () => {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        // Update feature
        const updatedFeature = featureStorage.updateFeature(featureName, updatedData, {
          updatedBy: 'test',
        });
  ...
  test('should handle non-existent features gracefully', () => {
    ...
    expect(() => featureStorage.updateFeature('non_existent', [1, 2, 3])).toThrow(
      'Feature not found: non_existent'
    );
