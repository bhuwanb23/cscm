...
describe('Feature Versioning Tests', () => {
  ...
  test('should validate required parameters', () => {
    ...
    expect(() => featureVersioning.createVersion(null, [1, 2, 3])).toThrow(
      'Feature name is required'
    );

    // Test getVersion validation
    expect(() => featureVersioning.getVersion(null, 1)).toThrow('Feature name is required');

    expect(() => featureVersioning.getVersion('feature', 0)).toThrow(
      'Valid version number is required'
    );

    // Test compareVersions validation
    expect(() => featureVersioning.compareVersions(null, 1, 2)).toThrow('Feature name is required');

    expect(() => featureVersioning.compareVersions('feature', 0, 2)).toThrow(
      'Valid version numbers are required'
    );
  });
});
