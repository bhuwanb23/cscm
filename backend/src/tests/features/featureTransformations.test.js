...
describe('Feature Transformations Tests', () => {
  ...
  test('should standardize data correctly', () => {
    ...
    const variance =
      standardized.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / standardized.length;
  ...
  test('should handle edge cases and errors', () => {
    ...
    expect(() => featureTransformations.normalize(['a', 'b', 'c'])).toThrow(
      'All elements must be numbers for normalization'
    );

    // Test log transform with negative values
    expect(() => featureTransformations.logTransform([-1, 0, 1])).toThrow(
      'All elements must be positive numbers for log transformation'
    );

    // Test unsupported encoding method
    expect(() => featureTransformations.encode([1, 2, 3], { method: 'unsupported' })).toThrow(
      'Unsupported encoding method: unsupported'
    );

    // Test unsupported imputation strategy
    expect(() => featureTransformations.impute([1, 2, 3], { strategy: 'unsupported' })).toThrow(
      'Unsupported imputation strategy: unsupported'
    );
  });
});
