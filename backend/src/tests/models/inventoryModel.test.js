...
describe('Inventory Model Tests', () => {
  ...
  test('should validate required fields for upsert', async () => {
    ...
    await expect(InventoryModel.upsert(inventoryData)).rejects.toThrow(
  ...
  test('should validate store ID for get by store', async () => {
    await expect(InventoryModel.getByStore(null)).rejects.toThrow('Store ID is required');
  });
  ...
  test('should validate required fields for get', async () => {
    await expect(InventoryModel.get(null, 'TEST-STORE')).rejects.toThrow(
      'Product ID and Store ID are required'
    );
  });
});
