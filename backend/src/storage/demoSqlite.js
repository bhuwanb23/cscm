...
async function demoSqliteDatabase() {
  ...
  try {
    ...
    inventoryItems.forEach((item) => {
      ...
      console.log(
        `  ${item.product_id}: ${item.quantity} units (${available} available, ${item.reserved_quantity} reserved)`
      );
    });
    ...
    deliveredShipments.forEach((shipment) => {
      console.log(
        `  ${shipment.shipment_id}: ${shipment.order_id} delivered to ${shipment.to_location}`
      );
    });
    ...
    console.log(
      '💡 This demonstrates a complete inventory, order, and shipment workflow using SQLite.'
    );
  } catch (error) {
